...
file_map = {
    ...
    '45_copy_constructor.cpp': '45_copy_constructor.cpp', # Inserted entry placeholder
...
# Generate isolated copy_constructor config
copy_const_data = """        { id: '45_copy_constructor.cpp', unit: 'Unit III: OOP', title: '45. Copy Constructor', concept: 'Memory Cloning', description: 'Exploring explicit reference passing to bypass infinite instancing loops during copy routines.', useCase: 'Object duplication, pass-by-value architectures.', bioNote: 'Enables explicit cloning of DNA fragments into sequence vectors for targeted structural edits.' }"""

# Inject before destructor
old_destructor_block = "{ id: '43_destructor.cpp'"
c = c.replace(old_destructor_block, copy_const_data + ",\n        " + old_destructor_block)

for old, new in file_map.items():
    if old == '45_copy_constructor.cpp':
        continue # Already injected as 45 natively
        
    old_num = old.split('_')[0]
    new_num = new.split('_')[0]
    c = c.replace(f"id: '{old}'", f"id: 'TMP_{new}'")
    c = c.replace(f"title: '{old_num}. ", f"title: 'TMP_{new_num}. ")
...
print("data.js successfully mapped and sequentially traced.")
